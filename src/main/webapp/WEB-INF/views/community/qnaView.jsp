...
<hr style="margin-top:20px;"  />  
	...
	<div id="boardViewDesc">
		<div><h4><%= qview.getCoTitle() %></h4></div>
		<hr style="margin-top:20px;"  />  
		<div><span id="boardViewWriter">글쓴이:&nbsp;&nbsp;<%=qview.getCoWriter() %></span> <br> <span id="boardViewRegDate"><%=new SimpleDateFormat("yyyy-MM-dd HH:mm").format(qview.getCoRegdate()) %></span></div>
	</div>
...
<hr />
	...
	<!-- 로그인 했든안했든 일단 폼은 보여줌 -->
	    <div class="comment-editor">
	        <form
	            action="<%=request.getContextPath()%>/community/communityCommentEnroll?co_type=Q" 
	            name="boardCommentFrm"
	            method="post"  >
	              	<input type="hidden" name="coNo" value="<%=qview.getCoNo() %>" /> <%--게시글번호 --%>
